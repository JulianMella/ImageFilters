...
def gaussian_blur(image: np.array, expanded_image: np.array, kernel_matrix: np.array, kernel_matrix_size: int) -> np.array:
    ...
    # Extract dimensions and channels of the image
    image_height, image_width, _ = image.shape

    # Calculate the radius of the kernel matrix
    kernel_matrix_radius = kernel_matrix_size // 2

    # Generate indices for the sliding windows
    rows, cols = np.indices((image_width, image_height))

    # Define the sliding window indices by adding offsets based on kernel matrix radius
    row_offsets = np.arange(-kernel_matrix_radius, kernel_matrix_radius + 1)
    col_offsets = np.arange(-kernel_matrix_radius, kernel_matrix_radius + 1)

    # Calculate indices for the neighborhoods
    neighborhood_rows = rows.reshape(-1, 1, 1) + row_offsets
    neighborhood_cols = cols.reshape(-1, 1, 1) + col_offsets

    # Extract neighborhood regions using advanced indexing from the provided expanded_image
    neighborhood_red = expanded_image[neighborhood_cols, neighborhood_rows, 0]
    neighborhood_green = expanded_image[neighborhood_cols, neighborhood_rows, 1]
    neighborhood_blue = expanded_image[neighborhood_cols, neighborhood_rows, 2]

    # Perform element-wise multiplication with the kernel matrix
    red_sums = np.sum(neighborhood_red * kernel_matrix, axis=(2, 3))
    green_sums = np.sum(neighborhood_green * kernel_matrix, axis=(2, 3))
    blue_sums = np.sum(neighborhood_blue * kernel_matrix, axis=(2, 3))

# Assign the summed values to the corresponding pixel channels after normalization if needed
    image[:, :, 0:1] = red_sums
    image[:, :, 1:2] = green_sums
    image[:, :, 2:3] = blue_sums
